...
out_root <- file.path(formal_root, "main_simulation",
                      "four_function_beta0_0p5_100rep_perrep_tuned_intercept_svd")
...
run_one_replicate <- function(rep_id) {
  ...
  metrics[[length(metrics) + 1L]] <- metric_row(
    rep_id, "WS-SSGL", ws$pred_test, ws$beta_grid, NULL, NULL, dat,
...
aggregate_outputs <- function() {
  ...
  atomic_write_csv(metrics, file.path(out_root, "results",
                                      "beta0_0p5_100rep_metrics_by_replicate.csv"))
  atomic_write_csv(pips, file.path(out_root, "results",
                                   "beta0_0p5_100rep_pips_by_replicate.csv"))
  atomic_write_csv(selected, file.path(out_root, "results",
                                       "beta0_0p5_100rep_selected_configs.csv"))
  ...
  atomic_write_csv(metric_summary, file.path(out_root, "results",
                                             "beta0_0p5_100rep_metric_summary.csv"))
  ...
  atomic_write_csv(pip_summary, file.path(out_root, "results",
                                          "beta0_0p5_100rep_pip_summary.csv"))
  ...
  atomic_write_csv(sel_summary, file.path(out_root, "results",
                                          "beta0_0p5_100rep_config_frequency_summary.csv"))

  sink(file.path(out_root, "reports", "beta0_0p5_100rep_perrep_tuned_report.md"))
  cat("# beta0=0.5 four-function 100-rep per-replicate tuning simulation\n\n")
