...
root <- file.path(project_root, "new_method_comparison", "formal_simulation", "main_simulation", "correlated_predictor_rho07_beta0_0p5_50rep_perrep_tuned_intercept_svd")
out_root <- file.path(root, "mgwr_extension_R50")
...
existing_path <- file.path(root, "results",
                           "rho07_beta0_0p5_50rep_metric_summary_with_wsssgl_posthoc_theta_u.csv")
if (!file.exists(existing_path)) {
  existing_path <- file.path(root, "results", "rho07_beta0_0p5_50rep_metric_summary.csv")
}
...
report <- c(
  "# MGWR extension for rho=0.7 beta0=0.5 four-function R50 simulation",
  "",
  paste0("- Source root: `", root, "`"),
  paste0("- Output root: `", out_root, "`"),
  paste0("- Completed MGWR fits: ", completed, " / 50"),
