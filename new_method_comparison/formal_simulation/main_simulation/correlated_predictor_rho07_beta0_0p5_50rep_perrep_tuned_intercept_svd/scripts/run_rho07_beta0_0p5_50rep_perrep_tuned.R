...
out_root <- file.path(formal_root, "main_simulation",
                      "correlated_predictor_rho07_beta0_0p5_50rep_perrep_tuned_intercept_svd")
...
saveRDS(list(
  ...
  predictor_design = "AR1 correlated predictors with Sigma[i,j] = rho_x^abs(i-j)",
...
make_ar1_from_base <- function(z, rho) {
  sigma <- outer(seq_len(ncol(z)), seq_len(ncol(z)), function(i, j) rho^abs(i - j))
  z %*% chol(sigma)
}
...
make_data <- function(seed) {
  ...
  X_raw <- make_ar1_from_base(z, rho_x)
  ...
  y <- true_beta0 + rowSums(X * rbind(beta_train, beta_test)) + eps
  list(
...
aggregate_outputs <- function() {
  ...
  atomic_write_csv(metrics, file.path(out_root, "results",
                                      "rho07_beta0_0p5_50rep_metrics_by_replicate.csv"))
  atomic_write_csv(pips, file.path(out_root, "results",
                                   "rho07_beta0_0p5_50rep_pips_by_replicate.csv"))
  atomic_write_csv(selected, file.path(out_root, "results",
                                       "rho07_beta0_0p5_50rep_selected_configs.csv"))

  metric_cols <- c("beta0_hat", "beta0_error", "mspe", "rmse", "mae", "bias",
                   "beta_mise", "mise_global_only", "mise_spatial_only",
                   "mise_global_plus_spatial", "mise_null", "theta_mse",
                   "u_mise_x3_x6", "runtime_sec")
  ...
  atomic_write_csv(metric_summary, file.path(out_root, "results",
                                             "rho07_beta0_0p5_50rep_metric_summary.csv"))
  ...
  atomic_write_csv(pip_summary, file.path(out_root, "results",
                                          "rho07_beta0_0p5_50rep_pip_summary.csv"))

  selection_by_rep <- do.call(rbind, lapply(split(pips, list(pips$method, pips$replicate),
                                             drop = TRUE), function(x) {
    selected <- x$rb_pip >= 0.5
    target <- as.logical(x$target)
    ...
    data.frame(
      ...
      method = x$method[1],
      selection_target = x$selection_target[1],
      ...
      exact_recovery = identical(selected, target),
      false_positives = fp,
      false_negatives = fn,
      stringsAsFactors = FALSE
    )
  }))
  atomic_write_csv(selection_by_rep, file.path(out_root, "results",
                                               "rho07_beta0_0p5_50rep_selection_by_replicate.csv"))
  selection_summary <- do.call(rbind, lapply(split(selection_by_rep, selection_by_rep$method),
                                             function(x) {
    metrics <- c("selected_size", "TPR", "FPR", "FDR", "exact_recovery",
                 "false_positives", "false_negatives")
    do.call(rbind, lapply(metrics, function(v) {
      z <- as.numeric(x[[v]])
      data.frame(
        method = x$method[1],
        selection_target = x$selection_target[1],
        ...
        sd = sd(z, na.rm = TRUE),
        mcse = sd(z, na.rm = TRUE) / sqrt(sum(is.finite(z))),
        stringsAsFactors = FALSE
      )
    }))
  }))
  atomic_write_csv(selection_summary, file.path(out_root, "results",
                                                "rho07_beta0_0p5_50rep_selection_summary.csv"))
  ...
  atomic_write_csv(sel_summary, file.path(out_root, "results",
                                          "rho07_beta0_0p5_50rep_config_frequency_summary.csv"))

  sink(file.path(out_root, "reports", "rho07_beta0_0p5_50rep_perrep_tuned_report.md"))
  cat("# rho=0.7 beta0=0.5 four-function 50-rep per-replicate tuning stress test\n\n")
...
if (is.na(only_rep)) {
  aggregate_outputs()
  cat("Saved rho=0.7 beta0=0.5 50-rep per-rep tuned simulation to:\n",
      out_root, "\n", sep = "")
}
